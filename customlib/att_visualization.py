...
class RIMCell(tf.keras.layers.Layer):
    ...
    """
    This is the original implementation of the alpha_t-RIM with the addition of the possibility
    to extract the layers via a dictonary. See comments in the Class code for more info.
    """
    ...
    def input_attention_mask(self, x, hs, training=False):
        ...
        # output dict for the sparse tensor
        self.output_dict['sparse_tensor'] = tf.sparse.to_dense(sparse_tensor)

        mask_ = tf.sparse.to_dense(sparse_tensor)

        mask = tf.expand_dims(mask_, axis=-1)
        #output dict for the mask
        self.output_dict['mask'] = mask

        self.attention_prob = self.input_attention_dropout(
            tf.nn.softmax(attention_scores2, axis=-1), training=training)
        # output dict for the attention probability
        self.output_dict['attention_prob'] = self.attention_prob

        self.inputs_attention = tf.matmul(self.attention_prob, value_layer2)
        # output dict for the input attention
        self.output_dict['inputs_attention'] = self.inputs_attention
        self.inputs1 = self.inputs_attention*mask
        self.output_dict['inputs1'] = self.inputs1

        return self.inputs1, mask
...
class RIMCell2(RIMCell):

    """
    The RIM cell 2 is to override the original RIM to be able to extract the layers from
    the model and to use it then with the feature extractor.
    """

    def __init__(self, **params):
        super(RIMCell2, self).__init__(**params)

    def call(self, inputs, states, training=False):
        output, state = super().call(inputs, states, training=training)

        # use the output dict
        self.output_dict['output'] = output

        # return the output dict and the state
        return self.output_dict, state
